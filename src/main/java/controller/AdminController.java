...
public class AdminController {
    ...
    public static void discountCode(String[] parts, int i) {
        double percent = Double.parseDouble(parts[i + 1]);
        int capacity = Integer.parseInt(parts[i + 2]);
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy/MM/dd");
        formatter = formatter.withLocale(Locale.ENGLISH);
        LocalDate date = LocalDate.parse(parts[i + 3], formatter);
        DiscountCode discountCode = new DiscountCode(percent, capacity, date);
        CostumerController.giveDiscount(discountCode);
    }
